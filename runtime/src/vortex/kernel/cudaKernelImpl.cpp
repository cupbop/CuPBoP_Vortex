...
__attribute__((always_inline))
void __vx_dxa_issue_2d(unsigned int desc_slot, unsigned int barrier_id,
    ...
    unsigned int a0 = (unsigned int)vx_wgather((size_t)(uintptr_t)smem_addr,
                                                (size_t)meta,
                                                (size_t)coord0,
                                                (size_t)coord1);
    __asm__ volatile (
...
__attribute__((always_inline))
void __vx_barrier_arrive_and_wait(unsigned int bar_id, unsigned int num_warps) {
    vx_barrier(bar_id, num_warps);
}

// Async barrier arrive (non-blocking), returns phase
__attribute__((always_inline))
int __vx_barrier_arrive(unsigned int bar_id, unsigned int num_warps) {
    return vx_barrier_arrive(bar_id, num_warps);
}

// Async barrier wait (blocking until phase completes)
__attribute__((always_inline))
void __vx_barrier_wait(unsigned int bar_id, unsigned int phase) {
    vx_barrier_wait(bar_id, phase);
}

// Get packed barrier id for DXA
__attribute__((always_inline))
unsigned int __vx_dxa_barrier_id(unsigned int id) {
    return __local_group_id + (id << 8);
}

// Check if current warp is the DXA warp (first warp in CTA)
__attribute__((always_inline))
int __vx_is_dxa_warp(void) {
    if (__warps_per_group == 0) return 0;
...
__attribute__((always_inline))
unsigned int __vx_warps_per_group(void) {
...
__attribute__((always_inline))
unsigned int __vx_wgmma_encode_desc(unsigned long long base_addr,
...
// fragC: 8 floats (accumulator), fragA_meta/fragB_meta: 8 floats (descriptor metadata)
__attribute__((always_inline))
void __vx_wgmma_mma_ss(float* fragC, float* fragA_meta, float* fragB_meta) {
    ...
    fragC[0] = fd0; fragC[1] = fd1; fragC[2] = fd2; fragC[3] = fd3;
    fragC[4] = fd4; fragC[5] = fd5; fragC[6] = fd6; fragC[7] = fd7;
}

__attribute__((always_inline))
void* __vx_local_mem(void) {
    // Must offset by __local_group_id so each CTA gets its own shared memory region
    // Same as vx_spawn.h's __local_mem(size) but without size param (caller manages layout)
    return (void*)csr_read(VX_CSR_LOCAL_MEM_BASE);
}

// Local mem with per-CTA offset (matches vx_spawn.h __local_mem(size))
__attribute__((always_inline))
void* __vx_local_mem_offset(unsigned int size) {
    return (void*)((char*)csr_read(VX_CSR_LOCAL_MEM_BASE) + __local_group_id * size);
}