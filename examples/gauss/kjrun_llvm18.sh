...
/usr/bin/time -f "\n[initial compilation Time] %C\nReal: %E  User: %U  Sys: %S" \
${LLVM_PREFIX}/bin/clang++ -O0 -g -std=c++11  ./$KERNEL_CU -I../.. --sysroot=/ --target=x86_64-linux-gnu --gcc-install-dir=/usr/lib/gcc/x86_64-linux-gnu/13 --cuda-path=$CUDA_PATH --cuda-gpu-arch=sm_50 -L$CUDA_PATH/lib64 -lcudart_static -ldl -lrt -pthread -save-temps -v  || true 2>> timing_log.txt
...
/usr/bin/time -f "\n[CuPBoP kernel translation Time] %C\nReal: %E  User: %U  Sys: %S" \
$CuPBoP_PATH/build/compilation/kernelTranslator $KERNEL-cuda-nvptx64-nvidia-cuda-sm_50.bc kernel.bc 2>> timing_log.txt
# gdb --args $CuPBoP_PATH/build/compilation/kernelTranslator $KERNEL-cuda-nvptx64-nvidia-cuda-sm_50.bc kernel.bc

llvm-dis kernel.bc

echo "--- Translate the host bitcode by using CuPBoP's host translator"
/usr/bin/time -f "\n[CUPBoP host translation Time] %C\nReal: %E  User: %U  Sys: %S" \
$CuPBoP_PATH/build/compilation/hostTranslator $KERNEL-host-x86_64-unknown-linux-gnu.bc host.bc 2>> timing_log.txt
llvm-dis host.bc 
...
then
    ...
    /usr/bin/time -f "\n[kernel.bc backend compilation Time] %C\nReal: %E  User: %U  Sys: %S" \
    ${LLVM_PREFIX}/bin/clang++ ${VX_CFLAGS} ${VX_VXFLAGS} kernel.bc -c -o kernel.o > kernel.log 2>&1    
    echo "--- compiling kernel_wrapper.cpp"
    /usr/bin/time -f "\n[wrapper linking compilation Time] %C\nReal: %E  User: %U  Sys: %S" \
    ${LLVM_PREFIX}/bin/clang++ ${VX_CFLAGS} ${VX_VXFLAGS} --gcc-toolchain=${RISCV_TOOLCHAIN_FOLDER} ../vortex_debug/kernel_wrapper.cpp -c -o kernel_wrapper.o -save-temps -v  || true 2>> timing_log.txt
    ...
    then
        ...
        /usr/bin/time -f "\n[kernel linking Time] %C\nReal: %E  User: %U  Sys: %S" \
        ${LLVM_PREFIX}/bin/clang++ ${VX_CFLAGS} ${VX_VXFLAGS} --gcc-toolchain=${RISCV_TOOLCHAIN_FOLDER} kernel_wrapper.o kernel.o ${CuPBoP_PATH}/runtime/src/vortex/kernel/cudaKernelImpl_64.o -lm ${VX_LDFLAGS} -o kernel.elf 2>> timing_log.txt
    fi

    nm -C --defined-only -g kernel.elf > lookup_global_symbols.txt
    #${LLVM_PREFIX}/bin/llvm-objcopy -O binary kernel.elf kernel.out    
    OBJCOPY=${LLVM_PREFIX}/bin/llvm-objcopy /usr/bin/time -f "\n[binary file generation Time] %C\nReal: %E  User: %U  Sys: %S" ${VORTEX_HOME}/kernel/scripts/vxbin.py kernel.elf kernel.vxbin 2>> timing_log.txt
    ...
    /usr/bin/time -f "\n[host linking Time] %C\nReal: %E  User: %U  Sys: %S" \
    g++ -g -O0 -Wall -L../../build/runtime -L../../build/runtime/threadPool -L${VORTEX_PATH}/runtime/ -I${VORTEX_PATH}/kernel/include -o host.out -fPIC -no-pie host.o host_vortexrt.o  -lc -lvortexRuntime -lvortex -lThreadPool -lpthread -lcudart -L../../cuda-12.1/lib64 2>> timing_log.txt
    ...
    LD_LIBRARY_PATH=../../build/runtime/threadPool:${VORTEX_PATH}/runtime/simx:../../build/runtime:${LD_LIBRARY_PATH} ./host.out -f ../../data/gaussian/matrix208.txt
