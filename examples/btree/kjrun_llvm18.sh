...
######################### Default Varaibles #################################
DEVICE=vortex
KERNEL_CU="kernel/kernel_gpu_cuda_all.cu"
ARCH=64
...
${LLVM_PREFIX}/bin/clang++ -O0 -g -std=c++11  ./$KERNEL_CU ./util/main.o ./util/num/num.o --sysroot=/ --target=x86_64-linux-gnu -L$CUDA_PATH/lib64 --cuda-gpu-arch=sm_50  -lcudart_static -ldl -lrt -pthread -save-temps -v  || true
...
then
    ...
    ${LLVM_PREFIX}/bin/clang++ ${VX_CFLAGS} ${VX_VXFLAGS} --gcc-toolchain=${RISCV_TOOLCHAIN_FOLDER} ./kernel_wrapper.cpp -c -o kernel_wrapper.o -save-temps -v  || true  
    ...
    LD_LIBRARY_PATH=../../build/runtime/threadÍPool:${VORTEX_PATH}/runtime/simx:../../build/runtime:${LD_LIBRARY_PATH} ./host.out file ../../data/b+tree/mil.txt command ../../data/b+tree/command.txt > Batch_vortex_test1.log
