...
//  __constant__ float3 ff_flux_contribution_density_energy[1];
 ...
 __device__ float ff_variable[NVAR];
 __device__ float3 ff_flux_contribution_momentum_x[1];
 __device__ float3 ff_flux_contribution_momentum_y[1];
 __device__ float3 ff_flux_contribution_momentum_z[1];
 __device__ float3 ff_flux_contribution_density_energy[1];
...
__global__ void cuda_initialize_variables(int nelr, float *variables) {
  ...
  if (i==0)
  {
  //print every element of ff_variable, ff_flux_contribution_momentum_x, ff_flux_contribution_momentum_y, ff_flux_contribution_momentum_z, ff_flux_contribution_density_energy using for loop
  for (int j = 0; j < NVAR; j++)
  {
    if (ff_variable[j] > 0.5)
      printf("greater than 0.5, ff_variable[%d]: %f\n", j, ff_variable[j]);   
    printf("ff_variable[%d]: %f\n", j, ff_variable[j]);
  }
  for (int j = 0; j < 1; j++)
  {
    printf("ff_flux_contribution_momentum_x[%d]: %f %f %f\n", j, ff_flux_contribution_momentum_x[j].x, ff_flux_contribution_momentum_x[j].y, ff_flux_contribution_momentum_x[j].z);
    printf("ff_flux_contribution_momentum_y[%d]: %f %f %f\n", j, ff_flux_contribution_momentum_y[j].x, ff_flux_contribution_momentum_y[j].y, ff_flux_contribution_momentum_y[j].z);
    printf("ff_flux_contribution_momentum_z[%d]: %f %f %f\n", j, ff_flux_contribution_momentum_z[j].x, ff_flux_contribution_momentum_z[j].y, ff_flux_contribution_momentum_z[j].z);
    printf("ff_flux_contribution_density_energy[%d]: %f %f %f\n", j, ff_flux_contribution_density_energy[j].x, ff_flux_contribution_density_energy[j].y, ff_flux_contribution_density_energy[j].z);
  }
}
    ...
    //printf device variable addresses
    //printf("device variable addresses: %p\n", ff_variable);
}
...
__global__ void cuda_compute_step_factor(int nelr, float *variables,
  ...
  //if (variables[i]>0.5)
  //  printf("variables[%d]: %f\n", i, variables[i]);
  //print variables[0] to [100]
  /*
  if (i == 0)
  {
    for (j=0; j<10; j++)
...
__global__ void cuda_compute_flux(int nelr, int *elements_surrounding_elements,
  ...
  //printf("variables: %f %f %f %f %f %f %f %f %f %f\n", variables[89600], variables[89601], variables[89602], variables[89603], variables[89604], variables[89605], variables[89606], variables[89607], variables[89608], variables[89609]);
  //print i + VAR_DENSITY * nelr
  //printf("i + VAR_DENSITY * nelr: %d\n", i + VAR_DENSITY * nelr);                              
  ...
  //print velocity_i
  //printf("velocity_i: %f %f %f\n", velocity_i.x, velocity_i.y, velocity_i.z);
  ...
  float dde_p = density_energy_i + pressure_i;
  //print velocity_i, momentum_nb, pressure_nb de_p
  //  printf("de_p: %f\n", de_p);
  flux_contribution_i_density_energy.x = velocity_i.x * dde_p;
  flux_contribution_i_density_energy.y = velocity_i.y * dde_p;
  flux_contribution_i_density_energy.z = velocity_i.z * dde_p;
...
//#pragma unroll
  ...
  for (j = 0; j < NNB; j++) {
    ...
    if (nb >= 0) // a legitimate neighbor
    {
      ...
      //printf("velocity_nb: %f %f %f\n", velocity_nb.x, velocity_nb.y, velocity_nb.z);
      //printf("momentum_nb: %f %f %f\n", momentum_nb.x, momentum_nb.y, momentum_nb.z);
      //printf("pressure_nb: %f\n", pressure_nb);
      ...
      float de_p = density_energy_nb + pressure_nb;
      //print velocity_nb, momentum_nb, pressure_nb de_p
      //  printf("de_p: %f\n", de_p);
...
int main(int argc, char **argv) {
  ...
  //{
    ...
    //print h_ff_variable[1]
    //printf("h_ff_variable[00]: %f %p\n", h_ff_variable[0], &h_ff_variable[0]);
    // checkCudaErrors(
