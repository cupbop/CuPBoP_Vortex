...
######################### Default Varaibles #################################
DEVICE=vortex
KERNEL_CU=testcase1.cu
ARCH=64
#############################################################################

export VORTEX_SCHEDULE_FLAG=3
...
# ${LLVM_PREFIX}/bin/clang++ -O0 -g -std=c++11 --include ./vortex_headers_dummy.h ./$KERNEL_CU -I../.. --sysroot=/ --target=x86_64-linux-gnu --gcc-install-dir=/usr/lib/gcc/x86_64-linux-gnu/13 --cuda-path=$CUDA_PATH --cuda-gpu-arch=sm_50 -L$CUDA_PATH/lib64  -lcudart_static -ldl -lrt -pthread -save-temps -v  || true
...
then
    ...
    then
        ...
        VX_CFLAGS="-O0 --sysroot=${RISCV_TOOLCHAIN}/riscv64-unknown-elf --gcc-toolchain=${TOOLDIR}/riscv64-gnu-toolchain -march=rv64imafd -mabi=lp64d -mcmodel=medany -fno-rtti -fno-exceptions -nostartfiles -nostdlib -fdata-sections -ffunction-sections -I${VORTEX_HOME}/kernel/include -I${VORTEX_PATH}/kernel/../hw -DXLEN_64 -DNDEBUG"
    ...
    echo "--- Kernel link result debugging"
    ${LLVM_PREFIX}/bin/llvm-link kernel.bc kernel_wrapper.bc -o kernel_linked.bc
    ${LLVM_PREFIX}/bin/llvm-dis kernel_linked.bc
    ...
    LD_LIBRARY_PATH=../../build/runtime/threadPool:${VORTEX_PATH}/runtime/simx:../../build/runtime:${LD_LIBRARY_PATH} ./host.out  > host_out.dump
