...
######################### Default Varaibles #################################
DEVICE=vortex
KERNEL_CU="kernel/kernel_gpu_cuda_wrapper.cu"
ARCH=64
#############################################################################
...
gcc -g -c ./util/timer/timer.c -I./  -o ./util/timer/timer.o
gcc -g -c ./main.c -o ./main.o


# Possible to put -O3 here to generate simpler code
echo "--- Generate bitcode files(.bc) for host and device by using clang++"
${LLVM_PREFIX}/bin/clang++ -O0 -g -std=c++11  ./$KERNEL_CU ./main.o ./util/timer/timer.o ./util/num/num.o -I../.. --sysroot=/ --target=x86_64-linux-gnu --gcc-install-dir=/usr/lib/gcc/x86_64-linux-gnu/13 --cuda-path=$CUDA_PATH --cuda-gpu-arch=sm_50 -L$CUDA_PATH/lib64  -lcudart_static -ldl -lrt -pthread -save-temps -v  || true
