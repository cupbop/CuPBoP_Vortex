...
__global__ void calculate_temp(int iteration,   // number of iteration
  ...
  if (IN_RANGE(loadYidx, 0, grid_rows - 1) &&
    ...
    //printf("hello %d, %d\n", loadYidx, loadXidx);
    //printf("temp, power %f, %f\n", temp_on_cuda[ty][tx], power_on_cuda[ty][tx]);
  }
  __syncthreads();
...
int compute_tran_temp(float *MatrixPower, float *MatrixTemp[2], int col,
  ...
  printf("check: %d %d %d", BLOCK_SIZE, blockCols, blockRows);
...
#define EXPAND_RATE                                                            \
  ...
  printf("check2 %d, %d %d %d\n", grid_cols, grid_rows, smallBlockCol, smallBlockRow);
