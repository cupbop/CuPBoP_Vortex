...
__global__ void calculate_temp(int iteration,   // number of iteration
  ...
  int validXmin = (blkX < ty) ? -blkX : 0;
  int validXmax = (blkXmax > grid_cols - 1)
                      ? BLOCK_SIZE - 1 - (blkXmax - grid_cols + 1)
                      : BLOCK_SIZE - 1;

  float temp_tx[BLOCK_SIZE];
  bool computed = true; // iteration + 1357;
  //xxx for (int i = 0; i < iteration; i++) {
    computed = false;
    if (
	// IN_RANGE(tx, i + 1, BLOCK_SIZE - i - 2) &&
        // IN_RANGE(tx, i - 1, BLOCK_SIZE - i - 2) &&  
        // IN_RANGE(tx, i, BLOCK_SIZE - i - 2) &&

        IN_RANGE(tx, validXmin, validXmax) 
	) {
      computed = true;
      //xxx temp_tx[tx] = i + 1357;
    }
/*xxx
    __syncthreads();
    if (i == iteration - 1)
...
int compute_tran_temp(float *MatrixPower, float *MatrixTemp[2], int col,
  ...
  for (t = 0; t < total_iterations; t += num_iterations) {
    ...
    // calculate_temp<<<dimGrid, dimBlock>>>(
       ...
       //  step, time_elapsed);
    calculate_temp<<<dimGrid, dimBlock>>>(MIN(num_iterations, total_iterations - t), MatrixTemp[dst], col, row, borderCols);
